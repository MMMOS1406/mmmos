...
export async function nextwaveV2BuildStoryboardSemantic(script, deps) {
  ...
  const transformPairOf = (es2) => {
    for (const c2 of verifiedCalcs) {
      if (!TRANSFORM_MODELS.has(c2.model)) continue;
  ...
  const ROLE_LINE_NOUN = { rate: 'ANNUAL RATE', fee_rate: 'ANNUAL FEE', tax_rate: 'TAX RATE', inflation_rate: 'INFLATION', savings_rate: 'SAVED', match_rate: 'EMPLOYER MATCH', principal: 'STARTING AMOUNT', baseline_amount: 'CURRENT AMOUNT', income_amount: 'INCOME', upfront_cost: 'UP-FRONT COST', gap: 'DIFFERENCE', change_pct: 'CHANGE' };
  ...
  partition.forEach((pscene, si) => {
    ...
    let msMode = null;
    if (!usePair && ['outcome_comparison', 'trajectory_comparison', 'single_figure', 'explanation'].includes(intent)) {
    ...
    if (['presenter_hook', 'presenter_conclusion', 'explanation'].includes(intent)) {
      ...
      scene.flow = { amount_entity: m.id, movable_entities: [...new Set(es.filter(flowAmount).map((e) => e.id))], from: p.fromLabel, to: p.toLabel, from_proposed: !!from, to_proposed: !!to };
      ...
      if (usePair) {
        ...
        b = es.find((e) => START_ROLES.includes(e.role));
        ...
        const outs = es.filter((e) => e.role === 'outcome');
      ...
      if (dir !== 'flat') p.deltaTextOverride = `${dir === 'down' ? '-' : '+'}${b.kind === 'money' ? withBasis(fmtMoney(gapVal), valBasis) : (a.kind === 'duration' ? fmtDur(gapVal, a.unit) : String(gapVal))}${pctText}`;
      ...
      const cardLines = shownRoles.map((e) => {
        ...
        const amt = e.role === 'recurring_amount' && e.cadence ? withBasis(e.display, `per_${e.cadence}`) : e.role === 'outcome' && e.kind === 'money' ? withBasis(e.display, b) : e.role === 'gap' && e.kind === 'money' ? withBasis(e.display, b) : e.display;
        if (e.role === 'horizon') return { entity_id: e.id, text: `OVER ${e.display}` };
        let lab = dedupeLabel(e, e.label);
        if (!lab) lab = ROLE_LINE_NOUN[e.role] || (['outcome', 'result_duration'].includes(e.role) ? (METRIC_TITLE[id0.metric] || (e.role === 'result_duration' ? 'TIME TO GOAL' : 'RESULT')) : '');
      ...
      const resNames = [...new Set(shownRoles.filter((e) => RESULT_FIRST.includes(e.role)).map((e) => METRIC_TITLE[identityOf(e).metric] || cleanLabel(e.label) || null).filter(Boolean))];
      const purposeUp = upper(pscene.purpose, 44);
  ...
  const DUP = (t) => { const w = String(t).toLowerCase().replace(/[^a-z0-9$%. ]+/g, ' ').split(/\s+/).filter(Boolean).map((x) => NUMW[x] || x); for (let i = 0; i + 1 < w.length; i++) if (w[i] === w[i + 1] || (i + 3 < w.length && w[i] === w[i + 2] && w[i + 1] === w[i + 3])) return true; return false; };
  outScenes.forEach((sc) => {
    ...
    if (sc.card) {
      ...
      cd.lines.forEach((l) => {
        ...
        if (e.role === 'outcome' && e.kind === 'money') { const b = identityOf(e).basis; if (basisWord(b) && !l.text.startsWith(withBasis(e.display, b))) badScene(sc, 'card_time_basis_not_shown', { line: l.text }); if (!basisWord(b) && /^\S+\/(MO|WK|YR|DAY|QTR)\b/.test(l.text)) badScene(sc, 'card_time_basis_wrongly_shown', { line: l.text }); }
      });
