...
const decimalsOf = (raw) => { const m = String(raw || '').match(/\.(\d+)/); return m ? Math.min(2, m[1].length) : 0; };
...
export async function nextwaveV2BuildStoryboardSemantic(script, deps) {
  ...
  for (const m of mentions) {
    ...
    const cad0 = textCad ? { cadence: textCad, evidence: 'script_text', rejected: pa && pa.cadence && pa.cadence !== textCad ? pa.cadence : null }
  ...
  const mentionBasis = (id) => {
    ...
    if (e.role === 'outcome') return (e.cadence && e.metric !== 'total_paid' && e.metric !== 'end_value' ? `per_${e.cadence}` : null) || METRIC_BASIS[e.metric] || (e.metric === 'contribution' && e.cadence ? `per_${e.cadence}` : 'unspecified');
