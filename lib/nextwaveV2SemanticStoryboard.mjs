...
function runCalculations(passProposal, mentionsById) {
  ...
  for (const c of passProposal.calculations) {
    ...
    results.set(c.id, { id: c.id, model: c.model, output: c.output, value: chosen.value, convention: chosen.convention, verified, target: c.target, params, inputs: inputMentions, periodCadence, candidates: cands.map((k) => +k.value.toFixed(4)) });
...
export async function nextwaveV2BuildStoryboardSemantic(script, deps) {
  ...
  for (const m of mentions) {
    ...
    const e = {
      ...
      cadence: (pa && pa.cadence) || d.attached_cadence || (m.ctx && (m.ctx.freqRight || m.ctx.freqClause) ? (m.ctx.freqRight || m.ctx.freqClause).cadence : null),
  ...
  const mentionBasis = (id) => {
    ...
    if (e.role === 'recurring_amount') return `per_${e.cadence || 'month'}`;
    if (e.role === 'outcome') return METRIC_BASIS[e.metric] || (e.metric === 'contribution' && e.cadence ? `per_${e.cadence}` : 'unspecified');
  ...
  const identityOf = (e) => {
    const c = e.provenance.mention_id && calcsByTarget.get(e.provenance.mention_id);
    if (c) return { metric: c.model === 'arithmetic' ? `${c.params.op}_of_calculations` : c.output, unit: e.kind === 'duration' ? e.unit : e.unit || null, basis: calcBasis(c), horizon: horizonOf(e), source: 'verified_calculation', calc: c.id };
  ...
  partition.forEach((pscene, si) => {
    ...
    if (arms.length < 2 && (['single_figure', 'explanation', 'start_to_result', 'outcome_comparison'].includes(intent) || (intent === 'change_over_window' && !es.some((e) => e.role === 'change_pct')))) {
      const rec = es.filter((e) => e.role === 'recurring_amount' && e.kind === 'money' && e.cadence);
    ...
    if (intent === 'outcome_comparison' && arms.length >= 2 && arms.every((a) => a.pick)) {
    ...
    if (intent === 'trajectory_comparison' && arms.length >= 2 && arms.every((a) => a.pick)) {
    ...
    if (['presenter_hook', 'presenter_conclusion', 'explanation'].includes(intent)) {
      ...
      scene.comparison = { form: 'chart', metric: 'end_value', values: arms.map((a) => ({ entity_id: a.pick.id, identity: identityOf(a.pick), label: a.label, shown: a.pick.display })), delta: null, scenario_labels: series.map((x) => x.label) };
      ...
      const metric = fromToArms ? null : (idB.metric || idA.metric || b.metric || a.metric || null);
      ...
      const setup = intent === 'start_to_result' ? [] : sharedSetup(arms);
      if (intent === 'start_to_result') {
        const one = (role) => { const xs = entities.filter((e) => e.role === role); return xs.length && new Set(xs.map((e) => JSON.stringify(e.value))).size === 1 ? xs[0] : null; };
        const hz = one('horizon'), rt = one('rate') || es.find((e) => ['inflation_rate', 'tax_rate', 'rate'].includes(e.role));
      ...
      const headerFor = () => (intent === 'start_to_result' ? (START_HEADER[metric] || 'AMOUNT') : null) || HEADER_BY_METRIC[metric] || (b.role === 'result_duration' || b.kind === 'duration' ? 'DURATION' : basisWord(basis) ? `${({ day: 'DAILY', week: 'WEEKLY', month: 'MONTHLY', quarter: 'QUARTERLY', year: 'YEARLY' })[basisWord(basis)] || 'PERIODIC'} AMOUNT` : 'OUTCOME');
      ...
      if (dir !== 'flat') p.deltaTextOverride = `${dir === 'down' ? '-' : '+'}${b.kind === 'money' ? withBasis(fmtMoney(gapVal), valBasis) : (a.kind === 'duration' ? fmtDur(gapVal, a.unit) : String(gapVal))}`;
      ...
      const lines = shownRoles.map((e) => {
        const b = identityOf(e).basis;
        const amt = e.role === 'recurring_amount' && e.cadence ? withBasis(e.display, `per_${e.cadence}`) : e.role === 'outcome' && e.kind === 'money' ? withBasis(e.display, b) : e.display;
        return `${amt}${e.label ? ' ' + e.label : ''}`;
      });
      const gapHere = es.find((e) => e.role === 'gap');
      p = { treatment, text, title: upper(pscene.purpose, 44) || (gapHere ? (gapHere.label || 'TOTAL DIFFERENCE') : 'KEY FIGURES'), values: lines };
  ...
  outScenes.forEach((sc) => {
    const c = sc.comparison; if (!c) return;
    ...
    const cp = c.intent === 'start_to_result' ? { ok: ents.every((e) => e.kind === ents[0].kind && identityOf(e).unit === identityOf(ents[0]).unit), why: ['start and result are not the same kind of quantity'] } : compatible(ents, { needHorizon: c.form === 'bars' });
    ...
    if (c.form === 'bars') {
      ...
      if (c.delta) {
        ...
        if (basisWord(c.basis) && !c.delta.text.endsWith(basisSuffix(c.basis))) bad('delta_time_basis', { text: c.delta.text, basis: c.basis });
      }
      if (c.intent !== 'start_to_result' && rp.headerLabel && c.metric && HEADER_BY_METRIC[c.metric] && rp.headerLabel !== HEADER_BY_METRIC[c.metric]) bad('header_vs_metric', { header: rp.headerLabel, metric: c.metric });
    } else {
  ...
  return {
    ...
    calculations: calcDetails, rejected_calculation_proposals: rejectedProposals,
