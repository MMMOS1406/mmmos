...
export function nextwaveV2RecoverComparisonSecondSide(slots, screenType, scene, plan) {
  ...
  for (const u of otherUnits) {
    ...
    const pick = candidates.find((c) => c !== existing.primary_value);
    if (pick) {
      slots.push({ unit_index: u.__idx, label: typeLabel[valType(pick)] || 'VALUE', primary_value: pick, secondary_value: null });
      return slots;
    }
  }
  ...
  const secondReal = sameUnitCandidates.find((c) => c !== existing.primary_value);
  if (secondReal) {
    slots.push({ unit_index: existing.unit_index, label: typeLabel[valType(secondReal)] || 'VALUE', primary_value: secondReal, secondary_value: null });
  }
  return slots;
}
