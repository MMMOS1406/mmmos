...
export function buildShort({ format, words, assets }) {
  ...
  const DESK_TOP = 1300;
  const X = { stackA: 205, basket: 385, coin: 135, real: 385, char: 815 };
  ...
  const cam = makeCamera([
    ...
    { t: A.buy + 0.7, x: 400, y: 1000, z: 1.2 },
    { t: A.half + 0.4, x: 400, y: 1000, z: 1.2 },
    { t: A.rate + 0.2, x: 400, y: 960, z: 1.12 },
    { t: A.v57 + 0.5, x: 400, y: 960, z: 1.12 },
  ...
  function drawWorld(g, t) {
    ...
    // ── host (behind the desk) ───────────────────────────────────────────────
    const pose = t < A.y10 ? 'point' : t < A.rate ? 'card' : t < A.still ? 'compare' : 'react';
    const sprite = assets.pose(pose);
    if (sprite) {
      const sc = charH / sprite.h; const cw = sprite.w * sc; const enter = tw(t, 0.05, 0.9, ease.outBack); const bob = Math.sin(t * 1.9) * 3;
      g.save(); g.translate(X.char + (1 - enter) * 420, charBottom + bob); if (sprite.flip) g.scale(-1, 1);
      g.drawImage(sprite.canvas, -cw / 2, -charH, cw, charH); g.restore();
    }
    ...
    if (bIn > 0.01 && out0(t) < 0.999) {
      ...
      g.save(); g.globalAlpha = bIn * (1 - out); g.translate(out * 260, 0); g.translate(X.basket, baseY + 60 + (1 - bIn) * -70);
    ...
    if (sIn > 0.01) {
      ...
      g.save(); g.globalAlpha *= 1 - tw(t, A.cash - 0.2, 0.4, ease.linear); pr.pill(g, 505, lineY - 30, '$50,000 TODAY', { size: 24, fill: P.navy, align: 'center' }); g.restore();
