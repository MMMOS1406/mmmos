...
export const CALC_MODELS = {
  ...
  arithmetic: {
    ...
    outputs: ['value'],
    doc: 'Plain arithmetic on quantities/calculations. op is one of: sum (a+b), difference (|a-b|), multiply (a*b), ratio (a/b), share_of (a * b percent), per_period (an annual amount a spread over the periods in a year at `cadence`), annualize (an amount a paid every `cadence` period, totalled over a year).',
  },
};
export const ARITH_OPS = ['sum', 'difference', 'multiply', 'ratio', 'share_of', 'per_period', 'annualize'];
...
export function compute(model, p, output) {
  switch (model) {
    ...
    case 'arithmetic': {
      ...
      const v = { sum: a + b, difference: Math.abs(a - b), multiply: a * b, ratio: b !== 0 ? a / b : NaN, share_of: (a * b) / 100, per_period: a / periodsPerYear(p.cadence || 'month'), annualize: a * periodsPerYear(p.cadence || 'month') }[p.op];
