...
function extractMentions(text, deps) {
  ...
  // hyphenated durations used as adjectives: "a 30-year fixed", "a 15-year loan"
  for (const m of text.matchAll(/\b(\d+(?:\.\d+)?|[a-z]+)-(years?|months?|weeks?|days?)\b/gi)) {
    const s = m.index, e = m.index + m[0].length;
    if (overlaps(s, e)) continue;
    const v = wordOrDigit(m[1], deps);
    if (!(v > 0)) continue;
    mentions.push({ raw: m[0], start: s, end: e, kind: 'duration', value: v, unit: m[2].toLowerCase().replace(/s$/, '') });
    consumed.push([s, e]);
  }
  ...
  // "from 6.8 to 5.2 percent": a bare number joined by to/and/or/through to a number that
  // carries a percent unit shares that unit (grammatical coordination, not a sentence patch)
  for (const m of text.matchAll(/(?<![\w$.,])(\d+(?:\.\d+)?)\s*(?:to|and|or|through|–|-)\s*(\d+(?:\.\d+)?)\s*(?:%|percent\b|per cent\b)/gi)) {
    const s = m.index, e = m.index + m[1].length;
    if (overlaps(s, e)) continue;
    const v = parseFloat(m[1]); if (!(v > 0)) continue;
    mentions.push({ raw: m[1], start: s, end: e, kind: 'percent', value: v });
    consumed.push([s, e]);
  }
