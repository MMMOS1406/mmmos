...
const CADENCE_ABBR = { day: 'DAY', week: 'WK', month: 'MO', quarter: 'QTR', year: 'YR' };
...
const unitWord = (unit, n) => unit.toUpperCase() + (n === 1 ? '' : 'S');
const fmtDur = (v, unit) => (Array.isArray(v) ? `${v[0]}-${v[1]} ${unitWord(unit, v[1])}` : `${v} ${unitWord(unit, v)}`);
const escapeRe = (s) => s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
const titleCase = (s) => s.replace(/\b([a-z])/g, (m) => m.toUpperCase());
const upperBound = (v) => (Array.isArray(v) ? v[1] : v);
const toYears = (v, unit) => upperBound(v) * (YEARS_PER[unit] || 1);
const has = (re, s) => re.test(s || '');

function meaningEventPattern(raw) {
...
const withinRounding = (stated, derived) => Math.abs(stated - derived) <= Math.max(1, 0.0005 * Math.abs(derived));
...
const tagsOf = (text) => TAGS.filter(([, re]) => re.test(text)).map(([t]) => t);
